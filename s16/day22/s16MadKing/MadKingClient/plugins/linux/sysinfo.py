...
def collect():
    filter_keys = ['Manufacturer', 'Serial Number', 'Product Name', 'UUID', 'Wake-up Type']     # 定义要查的字段
    raw_data = {}
    ...
    for key in filter_keys:
        try:
            ...
            res_to_list = cmd_res.split(':')        # 变成列表
            if len(res_to_list) > 1:         # the second one is wanted string
                raw_data[key] = res_to_list[1].strip()  # 数据存入字典
            else:
                raw_data[key] = -1      # 没有数据就存个-1
