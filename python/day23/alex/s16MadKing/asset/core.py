...
class Asset(object):
    def __init__(self, request):
        ...
        self.mandatory_fields = ['sn', 'asset_id', 'asset_type']  # 用户发过来的数据必须包含这三个字段
        self.field_sets = {
            'asset': ['manufactory'],
    ...
    def data_is_valid(self):
        data = self.request.POST.get("asset_data")  # 获取数据
        if data:
            try:
                data = json.loads(data)     # 转成字典
                self.mandatory_check(data)  # 检查数据合法性
                self.clean_data = data
    ...
    def __is_new_asset(self):
        ...
        #if not hasattr(self.asset_obj, 'server'):  # new asset
            return True
        else:
            return False
    ...
    def __update_asset_component(self, data_source, fk, update_fields, identify_field=None):
        ...
        try:
            component_obj = getattr(self.asset_obj, fk) #nic_set
            if hasattr(component_obj, 'all'):  # this component is reverse m2m relation with Asset model
                objects_from_db = component_obj.all() #[nic_obj1,nic_obj2...]
                for obj in objects_from_db:
                    key_field_data = getattr(obj, identify_field) #真实的mac 00-50-56-C0-00-08
                    ...
                    if type(data_source) is list:
                        for source_data_item in data_source: #到客户端的数据 源里去找到跟 服务器数据库中key_field_data对应的条目
                            key_field_data_from_source_data = source_data_item.get(identify_field) #客户端的某条网卡数据的mac
                            ...
                            else:  # 代表数据源里连mac 的值 都没有 ，key field data from source data cannot be none
                    ...
                    # elif type(data_source) is dict: #deprecated 废弃
    ...
    def __filter_add_or_deleted_components(self, model_obj_name, data_from_db, data_source, identify_field):
        ...
        model_obj_name : NIC
         data_from_db,
         data_source : client data
        identify_field: mad_address
        '''
    ...
    def __delete_components(self, all_components, delete_list, identify_field):
        ...
        for obj in all_components:
            val = getattr(obj, identify_field)#macaddress
    ...
    def __compare_componet(self, model_obj, fields_from_db, data_source):
        ...
        for field in fields_from_db: # update_fields=['name', 'sn', 'model', 'macaddress', 'ipaddress', 'netmask',
            # 1.取出双方的字段值
            # 2.统一数据类型
            # 3.对比
            # 4.根据对比结果，确定是否更新数据库
            val_from_db = getattr(model_obj, field)   # 从数据库里拿到field
            val_from_data_source = data_source.get(field)   # 从客户发过来的数据
            if val_from_data_source:        # 如果客户端有数据
                ...
                if type(val_from_db) is int:        # 如果DB里的数据是INT就转成INT
                    val_from_data_source = int(val_from_data_source)
                elif type(val_from_db) is float:    # 如果DB里数据是FLOAT就转成FLOAT
                    val_from_data_source = float(val_from_data_source)
                elif type(val_from_db) is str:      # 如果DB里数据是STR就转成STR
                ...
                if val_from_db == val_from_data_source:  # this field haven't changed since last update
                    pass        # 如果数据一样什么也不做.
                    # print '\033[32;1m val_from_db[%s]  ==
                    # val_from_data_source[%s]\033[0m' %(val_from_db,val_from_data_source)
                else:
                    ...
                    db_field = model_obj.meta.get_field(field)
